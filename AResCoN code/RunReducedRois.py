...
def RealReduceRois (**kwargs) :  
    ...
    def CodeStorage ():
        ...
        rois_path  = kwargs.get("_roipath").replace('\\', '/')                       # paths here will be backslashes for windows but wont work with INSIDE fiji macro in windows unless they become forward
        rois_path  = '"' + rois_path + '/"'                                          # adding quotes to the path to make it string INSIDE the fiji macro. Add a frontslash in the end
        save_path  = kwargs.get("rr_sub_path").replace('\\', '/')                    # take the subdirectory path that corresponds to a planeN folder for roi
        save_path  = '"' + save_path + '/"' 
        last_image_name = listdir(kwargs.get("_foldpath_image"))[-1]                   # gets the name of the last image inside the folder with the images. If the title of the image running in the macro is the same to this, fiji shuts down
        percentage = int(kwargs.get("prcntg"))
        final_prc  = (100 - percentage) /100                                         # If the user has selected 20%, this will be a final 80% of the initial. Which is 0.8 out of 1
...
run("Fresh Start");"""
    ...
    def RunFiji (code_fiji):
            fiji_path = kwargs.get("_fiji_path")                                       # paths here will be backslashes for windows
            images_folder_path = kwargs.get("_foldpath_image").replace('/', '\\')         # paths here will be frontslashes for windows and must change to backslashes to fit for fiji batch-process-macro input for windows
