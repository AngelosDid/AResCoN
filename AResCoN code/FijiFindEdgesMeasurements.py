...
def GetMeasurementsOfRois (**kwargs) :  
    ...
    def CodeStorage ():
        ...
        main_edges_save_path  = kwargs.get("subdir_edge_path").replace('\\', '/')    # takes the subdirectory path of a particular planeN from FindEdgesResults
        main_edges_save_path  = '"' + main_edges_save_path + '/"' 
...
roiManager("Show All");
roiManager("Measure");
saveAs("Results", {main_edges_save_path} + title + ".csv");
wait(500);
...
run("Fresh Start");"""
    ...
    def RunFiji (code_fiji):
            ...
            if user_system == 'Windows' :
                ...
                time.sleep(0.3)
                # assert pyperclip.paste() == code_fiji, "Clipboard content mismatch"
